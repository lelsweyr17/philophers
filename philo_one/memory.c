...
int	get_heap_memory(t_all **all)
{
	...
	(*all)->monitor->last_eat = (long int *)malloc((*all)->philo->nbr_of_philos * sizeof(long int));
	if (!(*all)->time || !(*all)->fork || !(*all)->one || !(*all)->t || \
	!(*all)->monitor || !(*all)->monitor->write || 	!(*all)->monitor->last_eat)
		return (0);
	return (1);
}
